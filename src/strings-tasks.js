...
function sumOfCodes(str) {
  if (!str) return 0;
  return str.split('').reduce((a, curr) => a + curr.charCodeAt(), 0);
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str[i])) res += 1;
  }
  return res;
}
...
function isPalindrome(str) {
  const originalString = str.toLowerCase().replace(/\W/g, '');
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((a, curr) => {
    return curr.length > a.length ? curr : a;
  }, '');
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const res = [];
  arr.forEach((elem) => res.push(elem.split('').reverse().join('')));
  return res.join(' ');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (elem) => output[input.indexOf(elem)]);
}
